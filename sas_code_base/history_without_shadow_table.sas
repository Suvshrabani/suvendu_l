%macro history_without_shadow_table;
...
/*
proc append base=&library..&tablename._s (ALTER=&alterpass ) data=&source.(obs=0); run;
...
/*
	...
	*/
	...
	proc sql; create view &library..&tablename._h as select * from &library..&tablename.; quit;
/*
	
	* Table for latest rows ;
	proc sql;
	create view	&library..&tablename._l as
	select		*
	from		&library..&tablename
...
*/
	...
	proc freq data=&library..&tablename. noprint; table information_date/out=information_date_freq; run;
	proc sql; select max(information_date) into : information_date_max from information_date_freq; quit;
	proc sort data=&library..&tablename. (where=(information_date=&information_date_max.)) out=&library..&tablename._l; by information_date; run;


	data &library..&tablename._s(alter=&alterpass.); set &library..&tablename._l; stop; run;
